...
export function VideoPlayer({
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 sm:p-5 dark:border-zinc-800 dark:bg-zinc-950 shadow-sm space-y-4">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {hasSecondary && secondaryVideo && (
          <div className="inline-flex rounded-xl border border-zinc-200 bg-zinc-100/90 p-1 dark:border-zinc-800 dark:bg-zinc-900/90 shrink-0 shadow-2xs">
            <button
              type="button"
      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-xl border border-zinc-200 bg-black dark:border-zinc-800 shadow-md">
        <iframe
      ...
      <div className="rounded-xl border border-zinc-100 bg-zinc-50/70 p-3.5 sm:p-4 dark:border-zinc-900 dark:bg-zinc-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-mono text-[11px] font-extrabold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
          ...
          <h4 className="text-sm sm:text-[14.5px] font-bold text-zinc-950 dark:text-zinc-50">
            {activeVideo.title}
          </h4>
          {activeVideo.description && (
            <p className="text-xs sm:text-[13px] text-zinc-600 dark:text-zinc-300 leading-relaxed font-medium pt-0.5">
              {activeVideo.description}
            </p>
          )}
        </div>
        ...
        {hasSecondary && (
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-bold font-mono text-zinc-700 hover:text-black dark:text-zinc-300 dark:hover:text-white transition-colors shrink-0 sm:self-center border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-3.5 py-2 rounded-lg shadow-2xs"
          >
            <span>
              {activeTab === "part1"
